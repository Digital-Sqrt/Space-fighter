...
void Frame_CLK_Start(void) 
{
    ...
    Frame_CLK_CLKEN |= Frame_CLK_CLKEN_MASK;
	Frame_CLK_CLKSTBY |= Frame_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Frame_CLK_Stop
********************************************************************************
*
* Summary:
...
void Frame_CLK_Stop(void) 
{
    ...
    Frame_CLK_CLKEN &= (uint8)(~Frame_CLK_CLKEN_MASK);
	Frame_CLK_CLKSTBY &= (uint8)(~Frame_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Frame_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Frame_CLK_CLKEN & Frame_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Frame_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Frame_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Frame_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Frame_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Frame_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Frame_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Frame_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Frame_CLK_GetSourceRegister();
    uint16 oldDivider = Frame_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Frame_CLK_CLKEN & Frame_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Frame_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Frame_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Frame_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: Frame_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Frame_CLK_MOD_SRC |= modeBitMask & (uint8)Frame_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: Frame_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Frame_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Frame_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: Frame_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Frame_CLK_MOD_SRC & (uint8)(Frame_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: Frame_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Frame_CLK_GetDividerRegister();
    uint8 oldSrc = Frame_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Frame_CLK_MOD_SRC |= CYCLK_SSS;
        Frame_CLK_MOD_SRC =
            (Frame_CLK_MOD_SRC & (uint8)(~Frame_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Frame_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Frame_CLK_MOD_SRC & Frame_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Frame_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Frame_CLK_PHASE = clkPhase & Frame_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Frame_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Frame_CLK_PHASE & Frame_CLK_PHASE_MASK;
}
